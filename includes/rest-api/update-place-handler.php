<?php
function ept_pe_rest_api_update_place_handler($request){
  ...
  if (!empty($_FILES['post_images']['name'])) {
    ...
    for ($i = 1; $i <= count($files['name']); $i++) {
      if (!empty($files['name'][$i])) {
        ...
        $_FILES['place_image_single']['tmp_name'] = $files['tmp_name'][$i];
        $_FILES['place_image_single']['error'] = $files['error'][$i];
        $_FILES['place_image_single']['size'] = $files['size'][$i];

        $attach_id = media_handle_upload('place_image_single', 0);
    ...
    unset($_FILES['place_image_single']);
}
  ...
  if (!empty($request->get_param('existing_images'))) {
    ...
    foreach ($existing_images as $image_id) {
      if (is_numeric($image_id)) {
          $attach_ids[] = $image_id;
          if ($image_id == $request->get_param('primary_image_id')){
            $primary_id = $image_id;
          }
      }
    }
  ...
  if(
    ...
    $request->get_param('place_title') == null ||
    $request->get_param('place_location') == null
  )
  {
  ...
  if ($request->get_param('post_id') == 0){
    $post_id = wp_insert_post($place_post);
  }
  else {
    $place_post['ID'] = $request->get_param('post_id');
    $post_id = wp_update_post($place_post);
  }

  if ($post_id == 0) {
    $response['message'] = 'Failed to create place.';
    return new WP_REST_Response($response, 500);
  }
  if (!empty($attach_ids)) {
